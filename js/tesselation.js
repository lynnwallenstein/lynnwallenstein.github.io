...
var calcDelaunayTriangulation = (function () {
  ...
  function circumcircle(vertices, i, j, k) {
    ...
    yCenter = yDiffIJ > yDiffJK ? m1 * (xCenter - xMidIJ) + yMidIJ : m2 * (xCenter - xMidJK) + yMidJK
    xDiff = xJ - xCenter
    yDiff = yJ - yCenter
    // return
  ...
  return function (vertices) {
    ...
    for (i = indices.length; i--; edges.length = 0) {
      ...
      for (j = candidates.length; j--; ) {
        ...
        edges.push(candidates[j].i, candidates[j].j, candidates[j].j, candidates[j].k, candidates[j].k, candidates[j].i)
        candidates.splice(j, 1)
      }
    ...
    for (i = locked.length; i--; )
      if (locked[i].i < n && locked[i].j < n && locked[i].k < n) candidates.push(locked[i].i, locked[i].j, locked[i].k)
    // done
    return candidates
  }
})()
